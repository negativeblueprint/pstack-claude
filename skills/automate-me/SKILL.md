...
This skill orchestrates three others: an inline mining pass (see step 1), Claude Code's `anthropic-skills:skill-creator` (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
- Claude Code's built-in `skill-creator` skill: skill authoring process and writing guidelines.
