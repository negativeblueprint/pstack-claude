...
**Control skill.** Pick it by surface. Browser, Electron, and web UIs use `/run` plus the Browser tools (`navigate`, `read_page`, `computer`). CLIs and TUIs use `/run`. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] Triage every the automated reviewer and security-reviewer comment per `../references/automated review-triage.md`.
...
Each live lane runs on its own cloud VM at the PR head. Drive through the **run** skill (`/run`), plus the Browser tools for web and Electron UIs.
...
- [ ] the automated reviewer triage done.
