...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the cloud session's status via `ListAgents`. Transcript mtime is not liveness.
