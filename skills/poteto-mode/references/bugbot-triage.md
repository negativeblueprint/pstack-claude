...
- Skip when: The PR description, screenshots, design review, or nearby code makes the visual change explicit, and the the automated reviewer comment is only restating that a shared visual default changed.
...
- Confidence: candidate
- Skip when: the automated reviewer flags an export, component, helper, or file as unused, and `gt ls -s`, upper-stack diffs, or PR context shows it is used by a later PR in the stack.
