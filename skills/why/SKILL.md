...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, editor cursor location, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCP tools from your own tool list (`mcp__<server>__<tool>` entries), and use `ToolSearch` to discover deferred MCP tools before relying on them.
...
- `run_in_background`: `true`. Subagents inherit the parent's tools including MCP servers, which MCP-backed investigators need. Investigators still shouldn't write anything. Say so in the prompt; that's a posture, not a sandbox. To make it a sandbox, give the investigator agent a `tools` list without Write or Edit.
...
- Leave the tool set inherited. The synthesizer's quality check spot-verifies citations, which can require MCP access.
