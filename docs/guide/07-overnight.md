...
- `/loop` is Claude Code's built-in wake mechanism (the `loop` skill), not a pstack skill. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses it to re-check the finish condition on events or a heartbeat.
