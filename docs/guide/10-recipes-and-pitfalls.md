...
- **Treating `auto` as a model slug.** `auto` and `inherit` mean "omit the model field so the subagent inherits the parent chat model." [Setup](./01-setup.md) covers the roles.
