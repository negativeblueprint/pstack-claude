...
These are newer than the previous baseline and have not been read closely. They are the next `/p-update` run's queue.

| Upstream | Why it is open |
|---|---|
| `23a56e2` forge-neutral playbooks, Fable 5.1 defaults | Touches many playbooks. Needs a per-file judgment, not a bulk copy. |
| `73f8be4` disable model invocation for five skills | Needs a frontmatter comparison across the whole tree to know whether this fork already matches. |
| `f8abedd` every claim carries its evidence or its label | Needs reading in full before deciding what it changes here. |
| `f5bdd68` operator-neutral pronouns, in-chat status tick | This fork fixed README pronouns only, in `8e11b47`. The skill prose was not swept. |
